...
export default function InputField({
  ...
  const variantClasses: Record<string, string> = {
    outlined: `border ${invalid ? "border-red-500" : "border-gray-400"} rounded-lg`,
    filled: `${darkMode ? "bg-gray-800 text-white" : "bg-red-100 text-black"} rounded-lg`,
  ...
  return (
    <div className={`flex flex-col w-full mb-4 ${disabled ? "opacity-50" : ""}`}>
      {label && (
        <label className={`mb-1 font-medium ${darkMode ? "text-gray-200" : "text-gray-800"}`}>
          {label}
        </label>
      )}
      ...
      {helperText && !invalid && <span className="mt-1 text-sm text-gray-500">{helperText}</span>}
      {invalid && errorMessage && <span className="mt-1 text-sm text-red-500">{errorMessage}</span>}
    </div>
  );
}
